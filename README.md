...
| 17 | `gap_closure.py` | stdout | **The 2.4× gap, a-third-to-half closed.** The width probe's own per-width slope fits give an *empirical* capacity-degradation fit κ̄(w) ≈ 4.4 − 11.6·√w (seed-501 nets; seed-500 nets give 4.06 − 9.7·√w — row 19 measures the scatter). Feeding it back self-consistently, v ← Φ_det(w(v), κ̄(w(v))), moves the deterministic fixed point 4.54σ² → 6.11σ² (seed-501 fit) to 7.13σ² (seed-500 fit) at t₀=1e-4, and the resulting √w\* = 0.094–0.101 sits **inside** the measured support [0.03, 0.13] (interpolation, not extrapolation). Shrinks the unexplained factor 2.4 → 1.5–1.8 (a third to a half of the log gap); the rest = stochastic per-generation compounding, still open. Pure numerics on logged data. | — | ~2 min |
...
- **Honest gap, a third-to-half closed:** recursion Φ_det fixed point 4.54σ² vs measured ≈11σ² → factor 2.4. Self-consistent iteration with the empirical capacity fit κ̄(w) ≈ 4.4 − 11.6·√w gives v\* = 6.11–7.13σ² across probe seeds (t₀=1e-4), √w\* = 0.094–0.101 inside the measured support, form-insensitive (≤10% across five fits incl. fit-free interpolation); mixture test κ̄ 3.37±0.15 vs 3.34 predicted. Residual factor 1.5–1.8 (stochastic compounding) stays open.
