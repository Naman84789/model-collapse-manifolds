...
| 25b | `mnist_kbar_w.py` | `mnist_kbar_w.jsonl` | **κ̄(w) on REAL data — closes the "constants only on synthetic tubes" limitation.** MNIST 8×8 pixel manifold (measured intrinsic normal σ₀≈0.072, thin vs tangent ≈0.41). Identical extraction as row 24 with per-point kNN-PCA normal frames (k=96, kdim=12), normal-only fattening. κ̄ falls 2.01→1.59 as √w runs 0.071→0.231 (**21%**), tight fit κ̄=2.22−2.7√w (max resid 0.017, 2 seeds). Same functional form as the four synthetic geometries → the degradation is not a tube artifact. | (§floor) | ~3 min |
