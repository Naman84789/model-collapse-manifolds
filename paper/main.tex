...
\item \textbf{Falsifiable empirics}, each targeting a named claim:
  ...
  \item \emph{Floor anatomy \& intervention}: read at the ceiling the theorem
    ...
    $\sigma=0.05$ (against $58\%$ read under $p_t$, and with half the seed spread), the
...
measured drop shifts to $0.005$ percentage points. Since $r$ varies by at most $1.3\%$
within a geometry across every case we measured --- flat, singly curved and doubly curved
--- a degradation law measured under $p_t$ transfers to \eqref{eq:khat} with a shift of
under one point. This is what licenses the real-data claim: the MNIST $8\times8$
degradation of $21.1\%$ would read between $20$ and $21\%$ under the sampler-law probe for
any $r$-variation in the measured range, so we do not re-run it. We flag the assumption
...
forward-process probe, which is what one expects once the probe and the dynamics are
...
($\kb=3.3$, bound $5.5\,\sigsq$; six seeds). Both are proven lower bounds; the second is
...
$t_0=10^{-4}$ probe of Section~\ref{sec:floor}, whose ratio does rise monotonically; this
probe inherits its parent's truncation $t_0=5\times10^{-3}$, and the two agree on the
level of the ratio while differing on its weak trend, so we read a constant ${\approx}0.83$
here and do not claim a direction. The degradation law is not merely rescaled but
