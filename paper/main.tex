...
the ceiling itself degrades on fatter training pools, from $3.7$--$4.1$ at
$\sqrt w=0.03$ down to $2.8$--$2.9$ at $\sqrt w=0.13$ (ranges over two training
seeds). Over this range the points are well described by the two-parameter fit
...
from $4.5$ to $6.1$--$7.1\,\sigsq$ at $t_0=10^{-4}$ (the range across the two probe
seeds' fits), with the resulting pool width $\sqrt{w^*}\approx0.094$--$0.101$
...
$1.5$--$1.8$ --- a third to a half of the logarithmic gap. The remainder we attribute
to stochastic per-generation compounding that no deterministic pass captures; its
constant stays open.
...
factor $2.4$ above the iterated single-pass bound, of which a third to a half (in log)
is accounted for by feeding the measured capacity degradation $\kb(w)$ back
self-consistently (Section~\ref{sec:floor}) --- an empirical closure, not a derivation
--- leaving a $1.5$--$1.8\times$ stochastic-compounding residual unmodeled.
