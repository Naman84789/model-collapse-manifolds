...
performed: \eqref{eq:varode} is an identity for every $\epshat$, because one Euler step
enters the variance only through $\Cov(X,\epshat(X))$.
\end{lemma}
...
\]
No property of $\epshat$ beyond square-integrability is used, so this is exact for a
nonlinear estimator; the $+dt$ is the injected noise. Divide by $dt$, let $dt\to0$, and
substitute $\Cov(x,\epshat(x))=\hat\kappa\Var(x)$ from \eqref{eq:khat} to get
\eqref{eq:varode}. With $\hat\kappa=\kappa=s/\gamma^2$: $\kappa/s=1/\gamma^2$ and
