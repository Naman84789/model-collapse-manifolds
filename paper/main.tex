...
straight segment and $0.986$--$0.990$ on the codimension-9 ring, whose slope is read on
...
the mechanism. Where the normal coordinate is \emph{curved} the probes separate: $0.82$--
$0.86$ on the ring tube, whose radial normal curves in one direction, and $0.740$--$0.746$
on $S^2$, which curves in two. The ordering is consistent across all five widths, though
...
$29.8$ against $29.2$ on the sphere, and $45.7$ against $45.8$ on the codimension-9 ring,
...
(\texttt{poolwidth\_probe\_samplerlaw.py}; five widths, three seeds each) reproduces the
published forward-process points and gives a sampler-law ceiling $0.82$ to $0.86$ of them,
rising monotonically in $\sigma$. This probe inherits its parent's truncation
$t_0=5\times10^{-3}$, so its ceilings sit slightly above those quoted at $t_0=10^{-4}$ in
Section~\ref{sec:floor} ($3.25$ against $3.01$ at $\sigma=0.05$); the \emph{ratio} between
the two probes is what transfers, and it agrees to within $0.011$ across both truncations. The degradation law is not merely rescaled but
\emph{flatter}, $\kb(w)\approx3.65-8.33\sqrt w$ against $4.46-11.42\sqrt w$ (max residual
$0.032$ and $0.033$), so no single factor converts one into the other. Propagated through the
same closure, the deterministic fixed point moves from $5.9\,\sigsq$ ($5.1$--$7.1$ over
seeds) to $9.2\,\sigsq$ ($8.3$--$10.5$); the two seed bands do not overlap, so the shift is
not seed scatter.
...
therefore reaches $9.2\,\sigsq$ in Step~1 alone, against the $9.9\,\sigsq$ Step~3 reaches
by applying it afterwards --- the same destination by a shorter route, agreeing to $8\%$,
which is within the protocol differences between the two measurements (five widths and
three seeds here, six widths there). The residual factor of $1.2$--$1.4$ against the
...
\eqref{eq:khat} constrains, rather than under $p_t$, closes it to $1.2$--$1.4\times$ with
