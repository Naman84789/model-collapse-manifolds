...
\item \textbf{Falsifiable empirics}: a zero-fit prediction of the measured sampling floor
  ...
  head-to-head win over the state-of-the-art remedy; a replay-buffer control at the
  anchor's exact information budget, separating anchoring from replay; a subcritical-%
  fraction run exhibiting the law's no-plateau regime; a pixel-space MNIST recursion; and a
...
the general case replaces $s^2_g$ by $\eta_g=s^2_g+(a^2(t_0(g))-1)w_g$, which vanishes
under any annealing schedule and is absorbed into the fitted law at fixed $t_0$.) Let
...
\begin{lemma}[orthogonality shield; \emph{we prove}, first order]\label{lem:shield}
...
part. (The normal equations hold under $p_t$; along the sampler's own path the ensemble
deviates from $p_t$, and the cross term re-enters as the product of that deviation with
the band residual $r(t)$, whose measured smallness, $r^2\approx0.02$--$0.05$, keeps it
second order; Appendix~\ref{app:ode}.)
\end{lemma}
...
replay control of Section~\ref{sec:fix}), bracketing the clean-tube single-pass floor
$3.8\,\sigsq$ and its degradation-adjusted values.
...
estimator to first order (the residual only adds variance; the path-deviation cross
term is second order, the product of the ensemble deviation with the measured-small
band residual $r$), and Lemma~\ref{lem:limitode} transfers the
