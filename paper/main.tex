...
\paragraph{An honest gap, half closed (\emph{we model}).} Iterating the floor through
the recursion (every generation's pool is at least as wide as
...
(Appendix~\ref{app:details}) accounts for roughly half of the excess: the ceiling itself
degrades on fatter training pools --- \emph{we observe}
$\kb(w)=4.43-11.6\sqrt{w}$ across $\sqrt w\in[0.03,0.13]$ (max fit residual $0.02$) ---
so a network trained on self-generated, fattened data has a worse slope profile than
the clean-tube analysis assumes. Feeding this measured law back self-consistently,
$v\leftarrow\Phidet\big(w(v),\kb(w(v))\big)$, moves the fixed point to $6.1\,\sigsq$ at
$t_0=10^{-4}$ ($7.4\,\sigsq$ at $10^{-3}$) with no new parameters, and the resulting
pool width $\sqrt{w^*}=0.094$ lies \emph{inside} the measured support of the $\kb(w)$
law (interpolation, not extrapolation). The remaining factor $\approx1.5$--$1.8$ we
attribute to stochastic per-generation compounding that no deterministic pass captures;
its constant stays open.
...
factor $2.4$ above the iterated single-pass bound, of which roughly half is accounted
for by feeding the measured capacity-degradation law $\kb(w)$ back self-consistently
(Section~\ref{sec:floor}), leaving a $1.5$--$1.8\times$ stochastic-compounding residual
unmodeled.
...
The same probe's per-width slope fits give the capacity-degradation law
...
are a deficit $\approx-0.94$, a capacity-degradation term $\approx+0.7$, and a
residual injection $\approx+0.3$ to $+0.4$; their sum reproduces the measured net
$c$ pointwise ($+0.13$ derived vs.\ $+0.13$ measured at $w=0.0049$; $+0.10$ vs.\
$+0.10$ at $w=0.01$). Most of the apparent injection response is thus the
degradation of $\kb$ on fatter pools, and the residual injection is stable and
positive, as the residual-channel model (Appendix~\ref{app:residual}) requires. We trust this
decomposition mid-range only: at the thinnest width the $\sqrt w$ parametrization
makes $\kb'(w)$ diverge, an artifact of the fit, not a mechanism.
