...
\emph{flat} normal coordinate is. A curved one is not: for a radial coordinate in $\R^D$,
$dr$ carries an extra $(D-1)/(2r)\,dt$. Measuring the residual of \eqref{eq:varode} along
the sampler's own trajectory (\texttt{curved\_ode\_residual.py}) gives $+0.002$ on the flat
segment, confirming exactness there, and $-0.06$ to $-0.07$ on the ring --- the right
sign for that extra drift but $5$--$6\times$ its magnitude, so the curved-coordinate
correction is real, larger than the na"ive term, and not yet accounted for. On curved data
\eqref{eq:varode} should be read as the flat-coordinate identity plus a measured residual of
this size, which is one mechanism behind the $O(\sigsq/\rho^2)$ term of
...
along the way and rejected as too small: the radial It\^o correction $(D-1)/(2r)$ adds an
extra contraction $-(D-1)V/R^2$ to \eqref{eq:varode} on curved data, lowering the envelope
by only $0.7$--$1.7\%$, an order short of accounting for the discrepancy.
