...
singular limit the floor is \emph{closed form}---$1/(8\kb^2)$ in the network's slope cap
$\kb$, with no dependence on the sampling schedule---so a fixed model collapses without
bound as the manifold thins, and staying bounded requires capacity growing like $\sqrt{J}$
in the Fisher information $J$. Empirically the trap is two-horned: the stochastic sampler
sticks $\sim\!11\times$ too wide, while the deterministic one collapses to a point; neither
reaches the data. \emph{Constructively}, we prove that periodically re-matching the model's
output to a small, \emph{fixed} pool of real examples---matching their \emph{local width},
not re-inserting the examples themselves---makes the loop \emph{memoryless}, so error stops
compounding for every positive fraction of real data. Over a 20-generation head-to-head the
...
generations---\emph{model collapse}
...
periodically re-matching the model's output to a small, unchanging set of real examples: not
by copying the examples back into training, but by rescaling to \emph{their local width}.
The rest of this section makes both statements precise---the first is our \emph{negative}
result (a capacity floor), the second our \emph{constructive} one (a data-anchoring fix).
...
singular limit the \emph{peak} slope diverges---the precise sense in which infinite
...
statement (the width closes on itself), not a claim that the network is scalar, in the same
way an order parameter closes a mean-field recursion without the microscopic state being
low-dimensional.
\emph{Validation:} fixed points measured in one experiment type predict those of an
independent type within $1$--$13\%$ across six $(t_0,\lambda)$ cells
...
Under Assumptions~\ref{ass:a0}--\ref{ass:a2} the recursion is one-dimensional:
\begin{equation}
...
sticks fat at $10.8$--$13.2\,\sigsq$ (red). With the noise zeroed (deterministic
...
times ($\hat\kappa/\kappa=0.95$--$0.97$ for $t\ge0.1$) and then saturates at
...
$69$--$75\%$ of the measured floor at each of three truncation times, with the exact-score
...
equation, \textbf{no critical fraction---stability for every $\lambda>0$}; and with
...
itself fat ($7$--$20\times\sigsq$ across settings); its value is a $\approx2\times$
...
\citep{khelifa2026recursive}---rises to $47\sigsq$, then thins only to
...
fitted at one $t_0$ predicts fixed points at the other within $1$--$13\%$
...
\midrule
Ablation, fixed $t_0{=}10^{-4}$ & stochastic (noise on) & $10.8$--$13.2$ & 3 \\
 & deterministic (noise zeroed) & $0$ (point collapse) & 3 \\
\midrule
Pixel MNIST, $G{=}8$ & unanchored & $3.06$ & 3 \\
 & anchored & $1.66$ & 3 \\
 & true-data baseline & $1.34$ & --- \\
\midrule
Floor prediction & measured $\kb{=}3.9\Rightarrow\Phidet$ & $3.8$ (zero fit) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
(1)~Assumptions~\ref{ass:a0}--\ref{ass:a2} are measured, cross-validated laws about the
...
law-identification cells, the two-horned ablation, and pixel-space MNIST---chosen so that
...
$\delta/s$---injection. (Along the reverse path the ensemble deviates from $p_t$; the
...
thin ratios at $0.62$--$0.65$ on MNIST latents; per-axis matching achieves
$|\text{thin}-1|\le0.012$.)
...
$r^2_{\mathrm{res}}(t)\approx0.02$--$0.05$, flat in $t$ and in sample size, with
...
$\kappa(t)$ ($0.95$--$0.97$ for $t\ge0.1$), saturates at $\kb\approx3.9$ inside the
band, and never exceeds $\kappa$ ($n$-independent across $1$k--$16$k). The decomposition
