...
We flag the real weakness of this route rather than leave it to be found, and it is sharper
than a loose constant. $L$ is set by an essential infimum, so it is governed by rare
small-radius stragglers and not by the bulk: reading $a$ off the $0.01\%$ radial quantile
instead ($0.36$--$0.46$ near the floor) costs $12$--$19\%$. But the honest statement is
stronger. The sampler is driven by Gaussian noise, so its ensemble has full support, and on
full support $\Delta h=(D-1)/r$ is not Lipschitz at all. Corollary~\ref{cor:curved} does not
merely weaken for the sampler we actually run; its hypothesis fails outright. It applies to
an ensemble supported away from the medial axis, which is a conditioning, not a description
of the process.

Nothing of consequence follows for the floor, because the exact term is immune to this. Near
the medial axis the mass element is $r^{d_\perp-1}dr$ against an integrand $1/r$, so
$\E[1/r]$ converges for every codimension $d_\perp\ge2$ and $\Cov(r,1/r)=1-\E[r]\E[1/r]$
stays finite on full support. The two-moment form is therefore the operative one for any
statement about a real sampler, and Corollary~\ref{cor:curved} should be read as the
assumption-light companion it is: a bound one can quote without measuring anything, on an
ensemble one has first arranged to keep away from the medial axis. The exact term \eqref{eq:curvterm} has no such
...
We should be precise about how far that parallel goes, because the two uses of the reach are
not the same. Azangulov (personal communication) points out that their score-regularity
result is in fact reach-agnostic: what the proof uses is that the support decomposes into
charts $f_i(B_d(0,1))$ with each $f_i$ Lipschitz, and a lower bound on the reach enters only
as a convenient way to guarantee such a decomposition exists. Our use is not substitutable in
that way. \eqref{eq:reachbound} bounds the shape operator of the level sets of $h$, and it is
the reach itself, not a chart decomposition, that supplies $\|S\|\le1/\tau$. So the reach is
sufficient in both settings and necessary in neither obvious sense, but it is load-bearing
here in a way it is not there, and we would expect our constant to survive a weakening of the
hypothesis only if the weaker condition still controls $\Delta h$.
...
from $3.47$ to $3.37$. Width and skew supply $5.6\%$ of the ring's $3.6\to3.0$ gap; moments
...
real-vs-real baseline ($20.15$): unanchored above ($+2.3$, the predicted fattening),
...
\emph{above} it (off-manifold distance $22.5\pm0.5$, a deviation of $+2.3$, the
fattening the theory predicts, same sign in every seed), while the anchored recursion
sits \emph{below} it ($16.2\pm0.3$, a deviation of $-3.9$, mild contraction toward the
